...
# Filename       :    bert_span.py
# Abstract       :    BERT+SPAN for NER task.
...
if dataset_name == 'RESUME':
    ...
    do_lower_case = True
work_dir = './work_dirs/bert_span_%s'%dataset_name

max_len=256#input max len
loader = dict(
    type='NERLoader',
	truncation=False,
	stride=max_len-2,
...
train_pipeline = [
    ...
    dict(type='ToTensor',keys=['input_ids', 'attention_masks', "token_type_ids", "start_positions", "end_positions", "input_len"])
]
val_pipeline = [
    dict(type='NERTransform', label_converter=ner_converter),
    dict(type='ToTensor',keys=['input_ids', 'attention_masks', "token_type_ids", "start_positions", "end_positions", "input_len"])
...
model = dict(
	...
	decoder=dict(type='SpanDecoder',label_converter=ner_converter)
	)
