...
# Load ctl lib as global variable for multi-process
cur_path = os.path.realpath(__file__)  # Current file path
...
generate_func = lib.generate_result
generate_func.argtypes = [ctl.ndpointer(np.float32, flags='C_CONTIGUOUS'),  # score map
...
def post_east(cur_score_map,
    ...
    result = np.zeros((width * height, 9), dtype=np.float32)  # 8 for coordinates and 1 for confidence
    result_num = c_int()

    generate_func(cur_score_map.reshape(-1), cur_geo_map.reshape(-1), height, width, pool_ratio, scale_factor,
