...
class TwoStageEndToEnd(BaseEndToEnd):
    ...
    def forward_train(self,
        ...
        N, old_c, old_h, old_w = recog_feats.size()
        recog_feats = recog_feats.view(N, old_c, 1, old_h * old_w)
    ...
    def simple_test(self,
        ...
        det_results = self.roi_head.simple_test(feats, proposal_list, img_metas, rescale=rescale)
        if self.roi_head.with_mask:
            ...
            det_bboxes = [np.concatenate(box, axis=0) for box in det_results]
            det_masks = None
        ...
        _masks = None
        if rescale:
            _bboxes = [torch.from_numpy(per_bbox[:, :4] * img_metas[idx]['scale_factor']).to(img.device) for idx, per_bbox in enumerate(det_bboxes)]
            if self.roi_head.with_mask:
                _masks = [BitmapMasks(per_mask.astype(float), per_mask.shape[1], per_mask.shape[2]).resize(img_metas[idx]['img_shape'][:2]) for idx, per_mask in enumerate(det_masks)]
        else:
            _bboxes = [torch.from_numpy(per_bbox[:, :4]).to(img.device) for per_bbox in det_bboxes]
            if self.roi_head.with_mask:
                _masks = [BitmapMasks(per_mask.astype(float), per_mask.shape[1], per_mask.shape[2]) for idx, per_mask in enumerate(det_masks)]
        ...
        for batch_id in range(img.size(0)):
            ind, _ = np.where(recog_rois == batch_id)
